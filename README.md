...
`goaac` is a decoder-only, pure-Go AAC-LC package. The decoder core is a
source-shaped Go translation of FAAD2 `2.11.2`, with Go APIs for ADTS streams,
raw AAC-LC access units, and RTMP/FLV AAC audio-message bodies.
...
imply any patent license or patent non-infringement guarantee.
...
`testdata/aaclc` against S16 PCM SHA-256 values generated from the pinned FAAD2
reference. The same fixtures are decoded through ADTS, raw access-unit, and
RTMP/FLV AAC tag paths. The live integration test can also synthesize a fresh
fixture with `ffmpeg`, build a small native FAAD2 oracle from
`third_party/faad2`, and byte-compare the Go decoder output against it.
...
See `docs/reference.md`, `docs/test-vectors.md`, and `docs/parity-ledger.md`
for the source-truth record.
