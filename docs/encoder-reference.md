...
| `libAACenc/src/band_nrg.cpp:FDKaacEnc_CalcSfbMaxScaleSpec`, `FDKaacEnc_CalcBandEnergyOptimShort`, `FDKaacEnc_CalcBandNrgMSOpt` without log-data output | `internal/fdkaac/band_nrg.go` | Source-derived scalefactor-band max-scale, short-window energy, and mid/side energy hashes, unsupported log-data branch tests, invalid offset/scale transition tests, and zero-allocation guard under the `internal/fdkaac` wasm test binary. |
