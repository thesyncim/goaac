...
| `libAACenc/src/bitenc.cpp:FDKaacEnc_encodeGlobalGain`, `FDKaacEnc_encodeIcsInfo`, `FDKaacEnc_encodeSectionData`, `FDKaacEnc_encodeScaleFactorData`, `FDKaacEnc_encodeSpectralData` | `internal/fdkaac/bitenc.go` | Source-derived byte vectors for global gain, long/short ICS, section data, scalefactor/noise data, spectral payloads, a combined long channel-payload sequence, invalid transition tests, and zero-allocation guard under the `internal/fdkaac` wasm test binary. |
